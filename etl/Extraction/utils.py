...
class Extraction:
    ...
    def prepare_filesystem(self, source_keyword:str):
        if "data" not in os.listdir(self.extraction_main_path):
            ...
            path = os.path.join(self.extraction_main_path, "data")
            os.mkdir(path, 0o777)
        else:
            logger.info("data folder already exists continue...")
        
        
        if source_keyword not in os.listdir(os.path.join(self.extraction_main_path, "data")):
    ...
    def prepare_history(self):
        ...
        if "history.csv" not in os.listdir(self.extraction_main_path):
            logger.info("Creating metadata file history.csv")
            with open(os.path.join(self.extraction_main_path, "history.csv"), "w") as file:
                # init header
    ...
    search_keyword = "uk-road-safety-accidents-and-vehicles"
        ...
        try:
            ...
            file_path = os.path.join(self.extraction_main_path, "data", search_keyword)
    ...
    def update_history(self, file_name:str, source_name:str ,filesize:int, download_time:int, file_path:str):
        try:
            ...
            df = pd.read_csv(os.path.join(self.extraction_main_path, "history.csv"))
            logger.info(file_name)
            logger.info(source_name)
            cond = (df.file_name == file_name) & (df.source_name == source_name)
            df.loc[cond,'is_lastest'] = False
            df.to_csv(os.path.join(self.extraction_main_path, "history.csv"), index=False)
